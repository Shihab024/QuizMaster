...
const Dashboard = () => {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const perfResponse = await axios.get(
        `${process.env.REACT_APP_API_URL}/api/quizzes/performance/${user.uid}`
      );
      ...
      const quizResponse = await axios.get(`${process.env.REACT_APP_API_URL}/api/quizzes/`);
  ...
  const handleDeleteQuiz = async (quizId) => {
    ...
    if (window.confirm("Are you sure you want to delete this quiz?")) {
      try {
        await axios.delete(
          `${process.env.REACT_APP_API_URL}/api/quizzes/delete/${quizId}`,
          {
